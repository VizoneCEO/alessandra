...
// 1. Verificar sesión administrativa o de ayudante
if (!isset($_SESSION['user_id']) || !in_array($_SESSION['perfil_id'], [1, 6])) {
    echo json_encode(["success" => false, "message" => "Acceso no autorizado u operación denegada."]);
    exit();
}

require_once 'db_connect.php';

// Obtener el ID del estudiante desde la petición (NO desde la sesión)
$usuario_id = isset($_POST['id_usuario']) ? intval($_POST['id_usuario']) : 0;
$tab = isset($_POST['tab']) ? $_POST['tab'] : '';

if ($usuario_id <= 0) {
    echo json_encode(["success" => false, "message" => "Error: Estudiante no identificado."]);
    exit();
}
...
try {
    switch ($tab) {
        case 'tab1':
            // Asegurar que el ID no se inyecta y pertenece al estudiante real referenciado
            ...
            $fecha_nacimiento = get_post('fecha_nacimiento') ? : null;
    ...
    if ($stmt->execute()) {
        echo json_encode(["success" => true, "message" => "Sección guardada correctamente por administración."]);
    } else {
