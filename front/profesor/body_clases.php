...
if ($clase_seleccionada_id) {
    ...
    function get_items_y_calificaciones($conn, $categoria_id, $alumnos) {
        ...
        // Obtenemos todas las actividades de la categoría en una sola consulta
        ...
        // Obtenemos TODAS las calificaciones de TODOS los alumnos para estas actividades en UNA SOLA CONSULTA
...
<?php else: ?>
    ...
    <div class="card mb-4">
        <div class="card-header"><i class="fas fa-balance-scale me-2"></i>Paso 1: Define las Ponderaciones (la suma debe ser 100%)</div>
        <div class="card-body">
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="input-group">
                            <input type="number" class="form-control ponderacion-input" name="ponderacion[<?php echo $cat_nombre; ?>]" value="<?php echo $categorias[$cat_nombre]['ponderacion'] ?? 0; ?>" min="0" max="100" required>
                    ...
                    <div class="col-md-3 d-flex align-items-end">
                        <div>
                            <strong>Total: <span id="total-ponderacion">0</span>%</strong>
                            <button type="submit" class="btn btn-primary ms-3">Guardar Ponderaciones</button>
                        </div>
                    </div>
                </div>
            </form>
    ...
    <ul class="nav nav-tabs" id="myTab" role="tablist">
        <?php foreach($categorias_principales as $cat_nombre): ?>
            <li class="nav-item" role="presentation">
                <button class="nav-link <?php if($cat_nombre == 'Actividades') echo 'active'; ?>" data-bs-toggle="tab" data-bs-target="#tab-<?php echo strtolower($cat_nombre); ?>" type="button"><?php echo $cat_nombre; ?></button>
            </li>
        <?php endforeach; ?>
    </ul>
    ...
    <div class="tab-content card" id="myTabContent">
        ...
        <div class="tab-pane fade <?php if($cat_nombre == 'Actividades') echo 'show active'; ?> p-4" id="tab-<?php echo strtolower($cat_nombre); ?>" role="tabpanel">
            <?php if(!$categoria_id): ?>
                <div class="alert alert-info">Guarda las ponderaciones primero para poder añadir <?php echo strtolower($cat_nombre); ?>.</div>
            <?php else: ?>
                <form action="../../back/profesor_actions.php" method="POST">
                                ...
                                <tr>
                                    ...
                                    <?php foreach($data['items'] as $item): ?>
                                        <th class="text-center">
                                            ...
                                            <form action="../../back/profesor_actions.php" method="POST" onsubmit="return confirm('¿Eliminar este elemento y todas sus calificaciones?');" class="d-inline">
                                                <input type="hidden" name="action" value="delete_item">
                                                <input type="hidden" name="clase_id" value="<?php echo $clase_seleccionada_id; ?>">
                                                <input type="hidden" name="actividad_id" value="<?php echo $item['id']; ?>">
                                                <button type="submit" class="btn btn-sm btn-outline-danger border-0 p-0 ms-1" title="Eliminar"><i class="fas fa-trash"></i></button>
                                            </form>
                                        </th>
                                    <?php endforeach; ?>
                                    <th style="width: 200px;">
                                        <form action="../../back/profesor_actions.php" method="POST">
                                            ...
                                            <div class="input-group">
                                                <input type="text" name="nombre_item" class="form-control" placeholder="Nuevo..." required>
                    ...
                    <button type="submit" class="btn btn-success float-end mt-3"><i class="fas fa-save me-2"></i>Guardar Cambios en <?php echo $cat_nombre; ?></button>
                </form>
            <?php endif; ?>
        </div>
        <?php endforeach; ?>
