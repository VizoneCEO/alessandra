...
// 2. Comprobar si el usuario es Ayudante (perfil_id = 6)
if ($_SESSION['perfil_id'] != 6) {
    // If not Ayudante, redirect to index or their respective dashboard
    header("Location: ../../index.php");
    exit();
}
...
<head>
    ...
    <title>Panel de Ayudante | AF Luxury</title>
    <!-- Tailwind CSS -->
    ...
    <aside class="w-64 bg-zinc-950 text-white flex-shrink-0 flex flex-col border-r border-zinc-900 shadow-2xl z-20">
        ...
        <nav class="flex-1 px-4 py-8 space-y-2 overflow-y-auto">
            ...
            $nav_items = [
                ...
                'logs' => ['icon' => 'fa-clipboard-list', 'label' => 'Bitácora'], // Added for Ayudante
                'mensajes' => ['icon' => 'fa-envelope', 'label' => 'Mensajería'],
                // Removed Finanzas & Boletos if not allowed, but Plan didn't specify removing them entirely.
                // Keeping same modules as Admin for now, but Users module will be restricted.
            ...
            $unread_count = 0;
            if ($q_ur && $row_ur = $q_ur->fetch_assoc()) {
            ...
            foreach ($nav_items as $key => $item):
                ...
                <a href="dashboard.php?page=<?php echo $key; ?>"
                    ...
                    <?php if ($key === 'mensajes' && $unread_count > 0): ?>
                        <span
        ...
        <div class="p-6 border-t border-zinc-900 bg-zinc-950">
            <div class="flex items-center mb-4">
                ...
                <div class="overflow-hidden">
                    ...
                    <p class="text-[10px] text-zinc-500 uppercase tracking-widest">Ayudante</p>
                </div>
            </div>
    ...
    <main class="flex-1 flex flex-col relative overflow-hidden bg-white">
        ...
        <header class="h-20 flex items-center justify-end px-8 border-b border-zinc-100 bg-white">
            <h1 class="font-serif text-xl text-zinc-900 italic">
                Portal Ayudante Administrativo
            </h1>
        </header>
        ...
        <div class="flex-1 overflow-y-auto p-8 md:p-12 relative">
            <?php
            // Reuse admin body files for most things, BUT override 'usuarios' with local version.
            // If local version exists, use it. Else check admin folder.
            // Wait, standard include path search? No, must specify path.
            
            $allowed_pages = ['main', 'finanzas', 'boletos', 'usuarios', 'mensajes', 'ciclos', 'sucursales', 'materias', 'asignacion', 'alumno_setup', 'logs', 'reporte', 'contacto_estudiante'];

            if (in_array($page, $allowed_pages)) {
                if (file_exists('body_' . $page . '.php')) {
                    include 'body_' . $page . '.php';
                } else {
                    // Fallback to admin folder for non-restricted pages
                    // CAUTION: This means checking if those pages have restricted elements too.
                    // For now, only 'usuarios' is the critical one requested.
                    include '../admin/body_' . $page . '.php';
                }
            } else {
                include '../admin/body_main.php';
            }
            ?>
        </div>
