<?php
if (!isset($_SESSION['user_id']) || !in_array($_SESSION['perfil_id'], [1, 6])) {
    exit('No tienes permiso para ver esta sección.');
}

require_once '../../back/db_connect.php';

$id_alumno = isset($_GET['id_usuario']) ? intval($_GET['id_usuario']) : 0;

if ($id_alumno <= 0) {
    echo "<div class='p-8'><p class='text-rose-500'>ID de usuario inválido.</p><a href='dashboard.php?page=usuarios' class='underline'>Volver</a></div>";
    exit();
}

// Obtener nombre del alumno
$sql_name = "SELECT nombre_completo, CURP FROM Usuarios WHERE id = $id_alumno";
$res_name = $conn->query($sql_name);
if (!$res_name || $res_name->num_rows === 0) {
    echo "<div class='p-8'><p class='text-rose-500'>Usuario no encontrado.</p><a href='dashboard.php?page=usuarios' class='underline'>Volver</a></div>";
    exit();
}
$row_name = $res_name->fetch_assoc();
$nombre_alumno = htmlspecialchars($row_name['nombre_completo']);
...
$sql = "SELECT c.*, u.CURP as curp_usuario FROM Usuarios u LEFT JOIN Contacto_Alumnos c ON u.id = c.usuario_id WHERE u.id = $id_alumno LIMIT 1";
$res = $conn->query($sql);
if ($res && $row = $res->fetch_assoc()) {
    ...
    $c['tiene_alergias'] = 0;
    $c['curp'] = htmlspecialchars($row_name['CURP']);
}
...
<div class="mb-6 flex justify-between items-center">
    <div>
        <a href="dashboard.php?page=usuarios" class="inline-flex items-center text-sm font-medium text-zinc-500 hover:text-zinc-900 mb-2 transition-colors">
            <i class="fas fa-arrow-left mr-2"></i> Volver a Usuarios
        </a>
        <h3 class="font-serif text-3xl text-zinc-900 leading-tight">Expediente Médico y Contacto</h3>
        <p class="text-zinc-500 font-light text-sm mt-1">Auditando la información crítica de <strong class="text-zinc-800"><?php echo $nombre_alumno; ?></strong></p>
    </div>
</div>
    ...
    <div class="w-full md:w-1/4 bg-zinc-50 border-r border-zinc-100 p-4">
        <nav class="flex flex-col space-y-2" id="tabs-nav">
            ...
            <button onclick="openTab('tab4')" id="btn-tab4" class="tab-btn w-full flex items-center justify-between px-4 py-3 text-left text-sm font-medium text-slate-600 rounded-lg hover:bg-zinc-200 transition-colors">
                <span><i class="fas fa-briefcase-medical w-5 text-rose-500"></i> Médico</span> <?php echo checkIcon($tab4_ok); ?>
            </button>
    ...
    <div class="w-full md:w-3/4 p-6 lg:p-10">
        ...
        <div id="tab1" class="tab-content relative">
            ...
            <form id="form-tab1" onsubmit="saveTab(event, 'tab1')">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">CURP <span class="text-[10px] lowercase font-normal">(Vinculado a Usuarios)</span></label>
                ...
                <div class="flex justify-end">
                    <button type="submit" class="px-6 py-2 bg-zinc-900 text-white font-bold text-sm rounded-lg shadow hover:bg-zinc-800 transition-all">
                        <i class="fas fa-save mr-2"></i> Actualizar Identidad
                    </button>
                </div>
            </form>
        </div>
        ...
        <div id="tab2" class="tab-content hidden relative">
            ...
            <form id="form-tab2" onsubmit="saveTab(event, 'tab2')">
                ...
                <div class="flex justify-end">
                    <button type="submit" class="px-6 py-2 bg-zinc-900 text-white font-bold text-sm rounded-lg shadow hover:bg-zinc-800 transition-all">
                        <i class="fas fa-save mr-2"></i> Actualizar Localización
                    </button>
                </div>
            </form>
        </div>
        ...
        <div id="tab3" class="tab-content hidden relative">
            ...
            <form id="form-tab3" onsubmit="saveTab(event, 'tab3')">
                ...
                <div class="flex justify-end">
                    <button type="submit" class="px-6 py-2 bg-zinc-900 text-white font-bold text-sm rounded-lg shadow hover:bg-zinc-800 transition-all">
                        <i class="fas fa-save mr-2"></i> Actualizar Contacto
                    </button>
                </div>
            </form>
        </div>
        ...
        <div id="tab4" class="tab-content hidden relative">
            <div class="bg-rose-50 border-l-4 border-rose-500 p-4 mb-6 rounded-r">
                <h3 class="text-rose-800 font-bold mb-1"><i class="fas fa-exclamation-triangle mr-2"></i> Panel de Auditoría Médica</h3>
                <p class="text-rose-600 text-sm font-light">Como administrador, puedes ajustar esta información en caso de una actualización reportada por el tutor.</p>
            </div>
            ...
            <form id="form-tab4" onsubmit="saveTab(event, 'tab4')">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div class="md:col-span-2 bg-slate-50 p-4 rounded-lg border border-slate-100">
                        <div class="flex items-center justify-between mb-2">
                            <span class="font-bold text-slate-700 text-sm">¿Padece alguna enfermedad o tratamiento especial?</span>
                        ...
                        <div id="enf_details" class="<?php echo $c['padece_enfermedad'] ? '' : 'hidden'; ?> mt-4 border-t border-slate-200 pt-4 grid grid-cols-1 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Enfermedad / Tratamiento</label>
                            ...
                            <div>
                                <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Síntomas recurrentes</label>
                    ...
                    <div class="md:col-span-2 bg-slate-50 p-4 rounded-lg border border-slate-100">
                        <div class="flex items-center justify-between mb-2">
                            <span class="font-bold text-slate-700 text-sm">¿Requiere medicamentos?</span>
                            <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <div id="med_details" class="<?php echo $c['requiere_medicamentos'] ? '' : 'hidden'; ?> mt-4 border-t border-slate-200 pt-4">
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Especifique (medicamentos / tiempos)</label>
                    ...
                    <div class="md:col-span-2 bg-slate-50 p-4 rounded-lg border border-slate-100">
                        <div class="flex items-center justify-between mb-2">
                            <span class="font-bold text-slate-700 text-sm">¿Tiene alergias (médicas/alimentos)?</span>
                            <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <div id="ale_details" class="<?php echo $c['tiene_alergias'] ? '' : 'hidden'; ?> mt-4 border-t border-slate-200 pt-4">
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Especifique alergias</label>
                    ...
                    <div class="md:col-span-2 pt-4">
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2"><i class="fas fa-chalkboard-teacher mr-1 text-slate-400"></i> Notas para el personal docente</label>
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2"><i class="fas fa-ambulance mr-1 text-rose-500"></i> Protocolos de Emergencia Validados</label>
                ...
                <div class="flex justify-end">
                    <button type="submit" class="px-6 py-2 bg-rose-600 text-white font-bold text-sm rounded-lg shadow hover:bg-rose-700 transition-all">
                        <i class="fas fa-save mr-2"></i> Actualizar Protocolos
                    </button>
                </div>
            </form>
        </div>
        ...
        <div id="tab5" class="tab-content hidden relative">
            ...
            <form id="form-tab5" onsubmit="saveTab(event, 'tab5')">
                ...
                <div class="flex justify-end">
                    <button type="submit" class="px-6 py-2 bg-zinc-900 text-white font-bold text-sm rounded-lg shadow hover:bg-zinc-800 transition-all">
                        <i class="fas fa-save mr-2"></i> Actualizar Tutor
                    </button>
                </div>
            </form>
        </div>
...
<script>
    ...
    function saveTab(e, tabId) {
        ...
        formData.append('id_usuario', <?php echo $id_alumno; ?>);

        fetch('../../back/admin_actions_contacto.php', {
            method: 'POST',
            body: formData
        })
