...
if (!$clase_seleccionada_id) {
    echo '<div class="p-8 text-center border rounded-xl bg-zinc-50 border-zinc-200">
            <i class="fas fa-exclamation-circle text-4xl text-zinc-300 mb-4"></i>
            <p class="text-zinc-500">Por favor, selecciona una clase desde el panel de "Gestión de Materias" para ver su reporte.</p>
            <a href="dashboard.php?page=materias" class="mt-4 inline-block text-sm font-bold text-indigo-600 hover:text-indigo-800">
...
// Obtener detalles de la clase para el encabezado
$sql_info = "SELECT m.nombre_materia, u.nombre_completo as profesor, s.nombre_sucursal, ce.nombre_ciclo 
             FROM Clases c 
             JOIN Materias m ON c.materia_id = m.id 
             JOIN Usuarios u ON c.profesor_id = u.id 
             JOIN Sucursales s ON c.sucursal_id = s.id 
             JOIN Ciclos_Escolares ce ON c.ciclo_id = ce.id 
             WHERE c.id = $clase_seleccionada_id";
$info_clase = $conn->query($sql_info)->fetch_assoc();
...
// --- 2. FUNCIÓN DE CÁLCULO (IDÉNTICA A PROFESOR) ---
...
<div class="mb-4">
    <a href="dashboard.php?page=materias"
        class="text-xs font-bold uppercase tracking-widest text-zinc-400 hover:text-zinc-600 mb-2 inline-block transition-colors">
        <i class="fas fa-arrow-left mr-1"></i> Regresar a Materias
    </a>
    <h3 class="font-serif text-3xl text-zinc-900 mb-2">Reporte Ejecutivo de Materia</h3>
    <div class="text-sm text-zinc-500 font-light flex gap-4">
        <span><strong class="font-bold text-zinc-700">Materia:</strong>
            <?php echo htmlspecialchars($info_clase['nombre_materia']); ?>
        </span>
        <span><strong class="font-bold text-zinc-700">Profesor:</strong>
            <?php echo htmlspecialchars($info_clase['profesor']); ?>
        </span>
        <span><strong class="font-bold text-zinc-700">Sucursal:</strong>
            <?php echo htmlspecialchars($info_clase['nombre_sucursal']); ?>
        </span>
    </div>
</div>
                    ...
                    <tr class="bg-zinc-900 text-white text-[10px] uppercase tracking-wider font-light">
                        ...
                        <?php foreach ($categorias_principales as $cat_nombre): ?>
                            <th colspan="3" class="px-4 py-2 text-center border-r border-zinc-800 border-b border-zinc-800">
                                <?php echo $cat_nombre; ?>
                                <span class="opacity-50 block text-[9px]">(
                                    <?php echo number_format($ponderaciones[$cat_nombre]['ponderacion'] ?? 0, 0); ?>%)
                                </span>
                            </th>
                        <?php endforeach; ?>
